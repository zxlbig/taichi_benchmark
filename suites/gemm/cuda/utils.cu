...
void test_mysgemm_vnative(INT M, INT N, INT K, FLOAT alpha, FLOAT *A, FLOAT *B, FLOAT beta, FLOAT *C){
    cudaDeviceSynchronize();
    dim3 blockDim(32,32);
    dim3 gridDim(CEIL_DIV(M,32),CEIL_DIV(N,32));
    mysgemm_native<<<gridDim, blockDim>>>(M,N,K,alpha,A,B,beta,C);
    cudaDeviceSynchronize();
}
...
void test_kernel(int kernel_num,INT m,INT n,INT k,FLOAT alpha,FLOAT *A,FLOAT *B,FLOAT beta,FLOAT *C, cublasHandle_t err){
    switch (kernel_num){
        ...
        case -1: test_mysgemm_vnative(m,n,k,alpha,A,B,beta,C); break;
